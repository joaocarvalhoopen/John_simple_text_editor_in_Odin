...
HELP_TEXT := []string{
	...
	"",
	"CLIPBOARD",
	"  Ctrl+C / Ctrl+X / Ctrl+V use an internal clipboard, mirrored to the",
	"  system clipboard via OSC 52 (works across SSH on terminals that",
	"  honour it: xterm, kitty, wezterm, alacritty, foot, iTerm2, recent",
	"  gnome-terminal; for tmux: set -g set-clipboard on).",
	"  External paste (terminal's own Paste shortcut: Cmd/Ctrl+Shift+V,",
	"  Shift+Insert, middle-click) is captured via bracketed paste and",
	"  inserted as a single edit.",
	"",
	"PERSISTENCE",
